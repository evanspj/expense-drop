...
import fileFormatSmallImage from '@images/file_format_small_optimized.png';
import fileFormatLargeImage from '@images/file_format_large_optimized.png';
...
function FeatureCard({ label, icon, description }) {
  return (
    <li className="bg-white border border-gray-200 rounded-xl p-6">
      ...
      <div className="w-full max-w-[120px]">
        <p
          className={`inline-flex border  ${
            required
              ? 'bg-black ring-4 ring-gray-300 text-white'
              : 'border-gray-200'
          } bg-white rounded-lg font-semibold px-3 py-1`}
        >
          {name}
        </p>
      </div>
      ...
      <Popover className="relative">
        {({ open, close }) => (
          <AnimatePresence>
            <Popover.Button
              key="mobile-menu"
              className="relative border border-gray-200 bg-white rounded-lg px-3 py-2 z-40"
            >
              {open ? (
                <svg
                  width="15"
            ...
            <Popover.Panel
              ...
              className="absolute top-[120%] right-0 min-w-[180px] bg-white rounded-lg shadow-xl border border-gray-200 overflow-hidden p-1 Z-40"
            >
...
function SavedSessionsButton({ hasSessions, openModal, large }) {
  ...
  return (
    <button
      onClick={openModal}
      className={`inline-flex items-center  border border-gray-200 bg-white hover:bg-black hover:text-white rounded-lg ${
        large ? 'font-medium px-5 py-3' : 'text-sm font-semibold px-3 py-2'
      }`}
    >
      Saved Sessions
    </button>
...
function DesktopNavLink({ href, children }) {
  return (
    <a
      href={href}
      className="inline-flex items-center hover:bg-gray-200 rounded-lg font-medium px-2 py-1"
    >
      {children}
    </a>
  );
...
export default function LandingPage() {
  ...
  return (
    <main className="px-6 lg:px-10 2xl:px-0">
      <Head>
        <title>Expense Drop</title>
      </Head>
      <nav className="hidden sticky top-0 lg:flex justify-end items-center max-w-screen-xl mx-auto h-[80px] bg-gray-100  z-40">
      ...
      />
      <div className="pt-20 pb-10 lg:py-10 space-y-40">
        ...
        <section id="features" className="max-w-screen-xl mx-auto">
          <h2 className="font-bold text-2xl mb-10">Features</h2>
        ...
        <section id="file-format" className="max-w-screen-xl mx-auto">
          <h2 className="font-bold text-2xl mb-10">File Format</h2>
          <div>
            <div className="relative lg:hidden mb-10">
              <Image
                className="z-10 rounded-xl"
                src={fileFormatSmallImage}
                placeholder="blur"
              />
            </div>
            <div className="relative hidden lg:block mb-10">
              <Image
                className="z-10 rounded-xl"
                src={fileFormatLargeImage}
                placeholder="blur"
              />
            </div>
            <div className="lg:flex lg:justify-between">
              <p className="lg:w-1/2 textLg font-medium">
                For the app to work correctly, there is a specific format that
                is required for the excel file. The app expects the first row of
                the file to contain the headers for each column. The order of
                the columns does not matter, but the app will pull data from the
                headers <strong>date</strong>, <strong>description</strong>,{' '}
                <strong>account</strong>, <strong>category</strong>, and{' '}
                <strong>amount</strong> columns. Any other columns you have in
                your file will be ignored. An error will be displayed if you try
                to upload a file with them missing.{' '}
                <strong>Column headers need to be capitalized.</strong>
              </p>
              <div className="shrink mt-10 lg:mt-0 lg:w-1/2 lg:pl-10 xl:pl-16 space-y-3">
                <FileHeader
                  required
                  name="Date"
            ...
            <p className="w-full lg:w-1/2 mt-16 font-medium">
              <strong>NOTE:</strong> Though these headers are not required, if
              either the &quot;Account&quot; or &quot;Category&quot; value is
              left blank, it will affect how the data is presented. Ideally, you
              want to make sure all cell values are filled in.
            </p>
          </div>
        </section>
        <section className="max-w-screen-xl mx-auto h-80">
          <Dropzone />
          <div className="flex items-center space-x-4 mt-10">
            <button
              onClick={startDemo}
        ...
        <section id="faq" className="max-w-screen-xl mx-auto">
