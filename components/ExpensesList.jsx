...
function AccountLabel({ page, name }) {
  if (page && page === 'accounts')
    return (
      <p
        className='w-full flex items-center justify-end xl:justify-start xl:col-span-2 text-xs text-right xl:text-left text-gray-400 xl:text-gray-800 truncate xl:pl-4'>
        {name}
      </p>
    );
  return (
    <Link href={`/app/accounts/${nameToSlug(name)}`}
          className='w-full flex items-center justify-end xl:justify-start xl:col-span-2 text-xs xl:text-sm text-right xl:text-left text-gray-400 xl:text-gray-800 truncate hover:underline hover:text-indigo-500 outline-indigo-600 xl:pl-4'>
      {name}
    </Link>
  );
}
...
function CategoryLabel({ page, name }) {
  ...
  return (
    <Link href={`/app/categories/${nameToSlug(name)}`}
          className='bg-gray-200 text-xs rounded-md hover:bg-gray-800 hover:text-white  px-2 py-1'>
      {name}
    </Link>
  );
}
  ...
  return (
    <>
      ...
      <div className='flex-grow  lg:rounded-xl border border-gray-200 shadow-xl shadow-gray-500/10'>
        <div
          className='hidden xl:grid grid-cols-[50px_130px_repeat(10,minmax(0,1fr))] bg-gray-50 text-black border-b border-gray-200 rounded-t-xl text-xs px-4 py-3'>
